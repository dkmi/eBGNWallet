...
void SendFrame::parsePaymentRequest(QString _request) {
    if(_request.startsWith("ebgncoin://", Qt::CaseInsensitive))
    {
       _request.replace(0, 13, "ebgncoin:");
    }
    if(!_request.startsWith("ebgncoin:", Qt::CaseInsensitive)) {
      QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Payment request should start with ebgncoin:"), QtCriticalMsg));
      return;
    }
 ...
 ConfirmSendDialog dlg(&MainWindow::instance());
    ...
    if (dlg.exec() == QDialog::Accepted) {
      ...
      if (m_ui->donateCheckBox->isChecked()) {
          CryptoNote::WalletLegacyTransfer walletTransfer;
          walletTransfer.address = "VMpSoSKmnPYUaEJf1KiwZjP3WTrW1nZHDTN3ZT8cC2hkg5ehSLfteDZhjpmnZdfyGVfzE4pdTGnfEhnawNvan85j7uhsfLXUKW";
